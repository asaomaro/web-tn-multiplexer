...
## D5: 「自動」の行に添える行数は computed で出す（tasks の「開いたときに watch で読む」から変えた）

- 背景: tasks T9 は「開いたときに『自動』の行の N を読む処理を watch に足す」と書いた（design「振る舞いの詳細」の
  「N は開いたときの `effectiveScrollback("auto", kind, limit)`」から）。
- 決定: `computed(() => effectiveScrollback("auto", kind, session.limits.scrollbackLines))` にした。
- 理由: 入力（`kind` は起動時に決まった値、`limits` は reactive）から毎回求められるので、開いたときに写し取る必要が無い。
  写し取ると、開いている間にサーバの上限が届いた（再接続の snapshot）場合に古い値が残る。computed なら開いたときの値も、
  その後の変化も正しい。
- 影響: 単体テスト（`SettingsDialog.test.ts` の端末の節）は、開いたときの表示を見ているので design の意図どおりに通る。

## D6: 状態の印の blocked と idle の色を明るくし、記号を太字にする（以前の配色を変えた）

- 背景: review ラウンド1 が、線の細い字形のコントラストの不足を実測した。idle の ○（#6272a4）は通常の背景 #282a36 で 3.03:1、
  hover の #343746 で 2.51:1、選ばれている行の #44475a で 1.94:1。blocked の ×（#ff5555）も #44475a で 2.91:1。WCAG 1.4.11
  （非テキストのコントラスト 3:1）を下回り、塗りの丸よりインクが少ないぶん「色に頼らず形で読める」効果が落ちる。
- 決定: blocked を #ff6e6e、idle を #8a9ad0 にし、記号を太字にする。WCAG 2.x の式で計算したコントラスト比は、#ff6e6e が
  #282a36 で 5.23・#343746 で 4.33・#44475a で 3.36・モバイルのピッカーの #1e1f29 で 6.01、#8a9ad0 が同じ順に 5.16・4.28・3.32・5.93
  （**すべて 3:1 以上**。以前の #ff5555 は #44475a で 2.91、#6272a4 は 1.94）。working（#f1fa8c）と
  done（#50fa7b）は十分明るいので変えない。**記号「切」の丸にも同じ色が効く**（1 つの配色を保つ）。
- 理由 / 代替案: 退けた案: 記号のときだけ色を変える——同じ状態が設定で別の色になり、読み手を迷わせる。退けた案: 太字だけ——インクは
  増えるがコントラスト比は変わらない。herdr の既定の配色（Dracula）からは少し離れるが、この work の目的（色覚特性の利用者が状態を
  読める状態）を優先した。
- 影響: E2E（AC5）の色の期待を `rgb(255, 110, 110)` に直した。`docs/verification.md` の色の名前（赤・青灰）はそのまま当てはまる。
