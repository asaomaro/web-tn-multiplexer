...
`wtm serve --pane-history` で起動すると、各 pane の画面とスクロールバック（サーバのミラーが持つ分。色つき）を状態ディレクトリの
...
```bash
wtm serve --pane-history
# → wtm: 画面履歴を保存します（--pane-history）: <状態ディレクトリ>/session-history.json。pane の出力（秘密を含みうる）がディスクに残ります
```
