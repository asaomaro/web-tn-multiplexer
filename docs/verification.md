...
- **`wtm serve` を起動した端末を閉じると wtm も終わる**（SIGHUP。`nohup` でも同じ）。検証の途中で端末を閉じるなら
  tmux の中で動かす。
- **scrollback はデスクトップのブラウザで 5,000 行（`--scrollback` で変えられる。上限 10,000）、スマートフォン等の
  モバイルのブラウザで 1,000 行**（サーバのミラーは `--scrollback` の行数を持つ。メモリの目安は `docs/tls-setup.md`
...
- [ ] M7 の 3——pane の枠から戻す：`cat -v` を動かしたまま、pane の枠（端末の外周の幅 4px の縁。ポインタを重ねると色が付き、
      カーソルがメニューの形になる）を右クリックする。期待：行に何も出ずに pane のメニューが開き、項目が「herdr のメニューを使う」に
...
- [ ] scrollback の行数（AC5・D107）：pane で `seq 1 6000` を実行し、ホイールで一番上まで遡る。期待：先頭は `1` ではなく
...
- [ ] claude・codex 以外のエージェント（AC6。実物で確かめたのは Claude Code と Codex だけ）：`wtm serve` の起動時のログの行
      ...
      行が出て、名前の横に「未検証」と出る。行の丸は、動いている間は黄（working）、承認を求めると赤（blocked）、止まって入力を
      待つと青灰（idle）になり、別の pane を見ている間に終わると緑（done。その pane を表示すると idle に戻る）。行のクリックで
...
- [ ] AC6・AC7：Claude Code 等のエージェントを動かし、サイドバーに状態が出て、行のクリックでその pane へ移る。
- [ ] AC8：ブラウザを閉じて開き直す。期待：workspace・tab・pane の構成と画面（scrollback を含む）が戻る。あわせて、ページを
...
- [ ] AC13：`?` でヘルプ（キーの一覧）が開き、`/` で絞り込める。上の一巡で使わなかったキーも一覧どおりに動く：`H`/`J`/`K`/`L`
      ...
      当てていないことがある）では確かめず、Linux・WSL2 で確かめれば足りる）。後続に回したキー（`s` 等）は「未対応（後続: …）」と
      出る。
...
- [ ] 1 列のレイアウト：期待：上部のバー（「<workspace> / <tab>」・「この端末に合わせる」・⌨）の下に pane が 1 つだけ出て、
...
- [ ] pane ピッカー：バーの「<workspace> / <tab>」を押す。期待：全画面の一覧が出て、上に workspace とその tab の行（状態の
      丸つき）、下にエージェントの動いている pane の行（あれば）が並ぶ（エージェントの居ない pane そのものの行は無い——tab を選ぶと
...
- [ ] 縦スワイプでの scrollback（`mobile/TouchScroll.ts`。decisions.md D83：xterm.js 6.0.0 の
      ネイティブなタッチスクロールは壊れているため自前実装で補っている）が実機でも自然に動くか。モバイルで遡れるのは
      1,000 行まで（D107。デスクトップの 5,000 行より少ない）。
...
- [ ] 状態反映：`agent-detection.spec.ts` の 1 件が passed なら合格。その中で確かめているのは、判定のルールに当たる画面が
      ...
      claude・codex 以外のエージェントの項目や AC6・AC7 の一巡で、状態が変わってから丸の色が変わるまでに遅れを感じないことを
      目で見る。
