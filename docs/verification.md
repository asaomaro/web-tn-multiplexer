...
- **prefix と各操作のキーを、ブラウザごとに変えられる**（herdr のキー設定。20260921-keybinding-customization）：設定の節「キー」で、prefix と
  ...
  割り当てられない**。
- **tab バーと pane の枠の見た目を、ブラウザごとに変えられる**（herdr の `tab_bar_*`/`pane_*` 相当。
  20260922-tabbar-pane-appearance）：設定の節「表示」で、tab バーの位置（上/下）・「tab が 1 つなら自動的に
  隠す」・pane の枠の描画（自動/常に/なし）・pane 領域の外周の枠・pane 間の隙間・pane の枠へのエージェント名
  表示を選ぶと、その場で反映される（確定ボタンは無い）。tab バーの右端には、拡大の状態・接続先のホスト名・
  日時（4 つのプリセット）・固定文字列から選んで複数個・順序込みで並べられ、区切り文字も選べる（最大 16 件）。
  pane の枠・外周・隙間は既存の端末の大きさの計算に影響しない（見た目の色だけを切り替え、余白の数値は
  変えない）。既定値はほぼ今までと同じ見た目だが、herdr の既定に合わせたことで 2 点だけ新しく見た目が
  付く（意図的。docs/herdr-parity.md H23 を参照）：分割中の非選択 pane に薄い 1px の枠が新しく付く
  （選択中の pane の強調はそのまま）・pane 領域の外周に 1px の枠が新しく付く。
...
- [ ] tab バーと pane の枠の外観（20260922-tabbar-pane-appearance）：`prefix+s` で設定を開き、節「表示」で
      次を順に確かめる。（1）「tab バーの位置」を「下」にする。期待：tab バーが pane 領域の下へ移り、区切り線が
      上端に付く。（2）「tab が 1 つなら tab バーを自動的に隠す」を入にする。期待：tab が 1 つの間は tab バーが
      消え、`prefix+c` で tab を増やすと戻る（名前入力のダイアログは Enter で確定）。（3）右端エントリを
      「追加する種類」から選んで［追加］、日時なら書式の選択肢、固定文字列なら入力欄が行に出る。区切り文字の
      欄も変える。期待：tab バーの右端に選んだ内容が区切り文字でつながって出る。16 件追加すると［追加］が
      押せなくなる。［上へ］［下へ］で並びが替わり、［削除］した行の次の行の［削除］（無ければ［追加］）へ
      フォーカスが移る。（4）`prefix+v` で分割し、「pane の枠」を「自動」「常に」「なし」で切り替える。
      期待：「自動」「常に」は非選択の pane にも薄い枠が付き、「なし」は付かない（**単独 pane はその画面で
      唯一の pane なので常に選択中**——選択中の強調は `bordered` の値に関わらず優先するため、単独 pane の
      見た目はどの設定でも変わらない。違いが見えるのは分割中の非選択 pane だけ）。「pane 領域の外周の枠」
      「pane 間の隙間」を切り替える。期待：外周は pane 領域全体の縁の線が消える/戻る、隙間は分割線の色が地の色に
      沈む/戻る（太さは変わらない）。（5）pane を右クリック（か枠にフォーカスして Enter）→「名前の変更」で
      名前を付け、「pane の枠へエージェント名を表示」を入にする。期待：その pane の枠の左上隅に付けた名前が
      小さく出る（エージェント名しか無い pane では、そのエージェント名が出る）。（6）ページを開き直す。期待：
      すべての設定が保存されたまま最初から反映されている。
...
- **pane の枠・外周・隙間には自動のコントラスト調整をしない**（20260922-tabbar-pane-appearance の design「振る舞いの詳細」。herdr と同じ）。
  テーマを暗いものに替えても、枠・隙間の色（`--wtm-menu-border`）自体は見づらくならない範囲だが、将来の配色次第では調整が要るかもしれない。
- **pane の枠・外周・隙間は視覚（色・outline の有無）だけを切り替え、余白の数値は変えない**（20260922-tabbar-pane-appearance）。herdr は
  実際にレイアウトの余白を詰めるため、herdr と見比べると隙間を切ったときの詰まり方が本製品の方が控えめに見える（既存の pane の葉の大きさが
  そのまま PTY の cols/rows に直結する構造のため、幾何を変えると回帰する。design「依拠する既存の事実」）。
- **tab バー右端の日時は herdr の strftime の自由書式ではなく 4 つのプリセット**（時刻・時刻+秒・日付・日付+時刻）に簡略化している
  （20260922-tabbar-pane-appearance）。herdr にある `Command`（任意コマンドを定期実行して表示）種別は対象外——サーバでコマンドを実行する
  仕組みと権限の設計が要るため、独自コマンドのキー割り当て（H12）と同じ理由で見送った。
