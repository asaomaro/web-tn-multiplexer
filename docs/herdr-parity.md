...
| H22 | tab バーの位置・右端の状態表示・自動非表示 | 20260922-tabbar-pane-appearance（設定の節「表示」に足した。**herdr との違い**：① 右端のエントリは zoom・接続先ホスト名・日時・固定文字列の 4 種（herdr にある `Command` 種別〔任意コマンドを定期実行〕は対象外——サーバでコマンドを実行する仕組みと権限の設計が要る。20260921-keybinding-customization が対象外にした「独自コマンドのキー」と同じ理由）② 日時は herdr の strftime の自由書式ではなく `Intl.DateTimeFormat` ベースの 4 プリセット（時刻・時刻+秒・日付・日付+時刻）に簡略化（本製品は利用者が書く設定ファイルが無く、GUI の選択式の方が一貫する）③ ホスト名は**接続先サーバのホスト名**（`session.host.hostname`。ブラウザのタブタイトルで既に使用中）——herdr はクライアントローカルのマシンのホスト名で、指すものが違う（ブラウザには OS のホスト名を取る手段が無く、リモートアクセスという本製品の性質上こちらが有用と判断）④ 上限はエントリ数 16 件（herdr と同じ）・区切り文字は 8 文字（herdr に上限は無いが暴走防止で設けた）⑤ 追加・削除・並び替えは一覧＋ボタン（herdr は設定ファイルの配列を書く） | AC1〜AC4・AC9〜AC12・AC-I1〜AC-I5（同 work） |
| H23 | pane の枠・隙間・エージェント名表示の設定 | 20260922-tabbar-pane-appearance（設定の節「表示」に足した。**herdr との違い**：① `pane_borders`（自動/常に/なし）・`pane_outer_borders`・`pane_gaps` は**視覚のみ**で実装した——`padding`/`width`/`height` の数値は変えず、色・`outline`（`border` は使わない）の有無だけを切り替える（herdr は実際にレイアウトの余白を削る）。本製品は既存の pane の葉の大きさがそのまま PTY の cols/rows に直結する構造のため、幾何を変えると回帰する② `show_agent_labels_on_pane_borders` はトグル自体が「枠にラベルを出すかどうか」を決める——herdr は常時タイトル（OSC 0/2）を枠に出し、このトグルはエージェント名まで出すかを絞るだけ（本製品は現状ラベルを一切描いておらず、部分的に機能を足すより一貫性を優先した意図的な逸脱）③ ラベルの名前解決は既存の唯一の規則 `paneNameOf`（手動名→エージェント名→端末タイトル）をそのまま使う④ `pane_scrollbars`（インタラクティブなスクロールバー）は対象外——本製品にスクロールバーの UI 自体が無く、新規のウィジェット実装が要るため別 work とする | AC5〜AC9・AC-I1〜AC-I5（同 work） |
