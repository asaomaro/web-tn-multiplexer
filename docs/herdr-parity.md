...
| H33 | 名前付き session | 20260926-named-session（`wtm serve --session <名前>`・`wtm token reset --session <名前>`・`wtm session list [--json]`・`wtm session delete <名前>`。状態は herdr の `<config_dir>/sessions/<name>/` と同じ形で `<既定の状態ディレクトリ>/sessions/<名前>/`、既定の session は今までの状態ディレクトリのまま、`default` はその別名。**herdr との違い**：① 状態を分けるだけで pane・socket・token・ログがまるごと別になる（本製品の状態は状態ディレクトリ 1 つから組み立てられるため）② `--state-dir D` と併せると `D/sessions/<名前>/`（herdr は置き場所が固定）③ 名前の規則は herdr の 1〜64 バイトの ASCII 英数字と `.` `_` `-`・`.`/`..` 不可に、**先頭の `-`・末尾の `.`・Windows の予約名（`con`・`nul`・`com1` 等）の禁止を足した**（Windows ネイティブも対象のため。decisions D2）④ 動いているかは socket への接続ではなく `wtm.lock`（pid・ホスト名）で見る ⑤ **`session stop`・`session attach`・環境変数 `HERDR_SESSION` 相当は無い**（止めるのは Ctrl+C か pid への `kill`。別の session は別の URL〔ポート〕をブラウザで開く。画面での session の表示・切り替え・ポートの記憶とともに後続。`.aidev/backlog/product-roadmap.md`）⑥ 公式フック連携の Unix socket のパス長の上限を超える名前・`--state-dir` は起動前に断る） | AC1〜AC13（同 work） |
...
| H39 | エージェント自動化（`agent start`／`prompt --wait` 等） | 20260926-agent-automation-api・20260926-agent-prompt-send-keys（**一部対応**：`wtmctl agent list`／`agent get`／`agent wait`〔`--until` の繰り返し・既定 idle/done/blocked・`--timeout` 省略で無期限・`agent_not_running`〕／`agent read`〔末尾 N 行・ANSI 除去〕／`agent prompt`〔送る瞬間の bracketed paste のモードで包む・300ms の遅延 Enter・送信中の他の入力は Enter の後へ・`blocked` なら送らず `agent_blocked`・`--wait` は送信後 5 秒以内に working/blocked を観測できなければ `agent_prompt_stalled`〕／`agent send-keys`〔xterm の既定の符号化・不明なキーは `invalid_key` で何も送らない〕。対象は pane ID だけ。送信はサーバの RPC `agent.prompt`／`agent.send_keys`〔既存の WebSocket・認証・Origin 検査の上〕、待ち合わせは既存の `pane.agent_status_changed` の push を CLI 側で行う。本物のエージェントでの送信は未検証〔偽のエージェントの結合テストのみ〕。使い方と herdr との違いは `docs/wtmctl.md`。残り——`agent start`・`agent rename` と名前による対象指定／agent skill ファイル——は backlog の後続項目） | AC1〜AC11（20260926-agent-automation-api）・AC1〜AC15（20260926-agent-prompt-send-keys） |
