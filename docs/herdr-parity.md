...
| H22 | ~~tab バーの位置~~・右端の状態表示（時刻）・自動非表示 | 20260922-appearance-settings-rest（`TabBar.vue`。tab が1個のとき自動で隠す・右端に現在時刻）。**位置（上/下）の切り替え、時刻以外の状態表示（ホスト名・コマンド結果）は対象外のまま**（同 work の requirements「対象外」） | AC4〜AC8, AC-I6（同 work） |
| H23 | pane の枠・隙間・エージェント名表示の設定 | 20260922-appearance-settings-rest（`PaneFrame.vue`/`Splitter.vue`/`App.vue`/`SettingsDialog.vue`。枠・隙間の太さ3段階・エージェント名の可視表示 opt-in） | AC9〜AC12, AC-I1〜AC-I4（同 work） |
