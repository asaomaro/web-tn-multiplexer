...
| H23 | pane の枠・隙間・~~描画モード（自動/常に/無し）~~・外周・~~隙間の入切~~・エージェント名表示の設定 | 20260922-appearance-settings-rest（`PaneFrame.vue`/`Splitter.vue`/`App.vue`/`SettingsDialog.vue`。枠・隙間の太さ3段階・エージェント名の可視表示 opt-in）＋ PR #12 から選択的に取り込み（decisions.md [[D11]]）：pane 領域の外周の枠（`settings.paneOuterBorders`）。**枠の描画モード「自動」（分割時だけ表示）・隙間の入切は対象外のまま**——`PaneFrame.vue` は常に padding を確保する設計（太さを3段階で選ぶ）で、#12 の「表示/非表示を切り替える」設計とは両立しない。統合するには `PaneLayout.vue` への手入れが要り、`.aidev/backlog/product-roadmap.md` へ後続として残した | AC9〜AC12, AC-I1〜AC-I4（appearance-settings-rest）／[[D11]] |
